...
- [Generative AI agent metrics](#generative-ai-agent-metrics)
  - [Metric: `gen_ai.agent.steps`](#metric-gen_aiagentsteps)
...
### Metric: `gen_ai.agent.steps`

This metric is [recommended][MetricRecommended] for instrumentations that
can attribute steps to a specific agent within a single invocation.

This metric SHOULD be specified with [ExplicitBucketBoundaries] of
[1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024].

<!-- weaver .registry.metrics[] | select(.name == "gen_ai.agent.steps") -->
<!-- NOTE: THIS TEXT IS AUTOGENERATED. DO NOT EDIT BY HAND. -->
<!-- see templates/registry/markdown/snippet.md.j2 -->
<!-- prettier-ignore-start -->
...
| Name | Instrument Type | Unit (UCUM) | Description | Stability | Entity Associations |
| -------- | --------------- | ----------- | -------------- | --------- | ------ |
| `gen_ai.agent.steps` | Histogram | `{step}` | Number of steps a GenAI agent takes during a single invocation. [1] | ![Development](https://img.shields.io/badge/-development-blue) | |

**[1]:** Recorded once per agent invocation. A "step" is a framework-defined
unit of work that the agent itself performs during the invocation;
instrumentations MUST count only steps the agent itself authored.

Common framework-specific definitions:

- **ADK**: each [event](https://adk.dev/runtime/event-loop/#event) in
  the invocation context whose `event.author` equals the agent name
  (typically a model response, a tool call the agent issued, or an
  agent transfer the agent initiated). Tool results, which are
  authored by the tool rather than the agent, are not counted.
- **LangGraph**: each node execution the agent itself performed in
  the workflow graph.
- **LangChain agents**: each `AgentAction` or `AgentFinish` step the
  agent itself emitted.
- **CrewAI**: each agent turn the agent itself took within a task.

Steps emitted by sub-agents or by the surrounding workflow SHOULD
NOT be counted; each step SHOULD be recorded exactly once across
the full call tree.

Steps are counted regardless of outcome: a step that errored or
partially completed still consumed the agent's execution and SHOULD
be counted.

Step counts MUST NOT be compared across different frameworks. They
are only meaningful within the context of a single framework's
instrumentation, which is why instrumentations MUST document what
counts as a step for their framework.
