...
- [Generative AI workflow metrics](#generative-ai-workflow-metrics)
  ...
  - [Metric: `gen_ai.workflow.steps`](#metric-gen_aiworkflowsteps)
...
`gen_ai.workflow.steps` complements `gen_ai.workflow.duration` by counting the
number of discrete steps observed within a single workflow execution. It is
intended for instrumentations of agent frameworks (for example, ADK,
LangChain agents, CrewAI agents) that can attribute observable events to a
...
### Metric: `gen_ai.workflow.steps`

This metric is [recommended][MetricRecommended] for instrumentations that
can reliably attribute steps (events) to a specific workflow or agent.

A "step" is intentionally framework-defined; it typically corresponds to a
single observable event emitted within the workflow (for example, a tool
call, a tool result, or a model response in ADK; a node execution in
LangGraph). When the `gen_ai.agent.name` dimension is set, instrumentations
SHOULD only count steps attributed to that agent; events attributed to
sub-agents or to a parent workflow SHOULD NOT be counted.

This metric SHOULD be specified with [ExplicitBucketBoundaries] of
[1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024].

<!-- weaver .registry.metrics[] | select(.name == "gen_ai.workflow.steps") -->
<!-- NOTE: THIS TEXT IS AUTOGENERATED. DO NOT EDIT BY HAND. -->
<!-- see templates/registry/markdown/snippet.md.j2 -->
<!-- prettier-ignore-start -->
...
authored by that agent); events attributed to sub-agents or to a
parent workflow SHOULD NOT be counted.

This metric complements `gen_ai.workflow.duration` by giving operators
visibility into how "chatty" or compute-intensive a workflow is, and
is useful for capacity planning and regression detection across
versions.
