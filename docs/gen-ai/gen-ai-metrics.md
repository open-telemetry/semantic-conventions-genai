...
- [Generative AI workflow metrics](#generative-ai-workflow-metrics)
  ...
  - [Metric: `gen_ai.invoke_workflow.inference_calls`](#metric-gen_aiinvoke_workflowinference_calls)
  - [Metric: `gen_ai.invoke_workflow.tool_calls`](#metric-gen_aiinvoke_workflowtool_calls)
...
The call count metrics below record a call made in a nested workflow against
both its own workflow execution and every enclosing workflow execution. See
[Workflow call count examples](./non-normative/examples-workflow-call-counts.md).
...
### Metric: `gen_ai.invoke_workflow.inference_calls`
...
<!-- weaver .registry.metrics[] | select(.name == "gen_ai.invoke_workflow.inference_calls") -->
<!-- NOTE: THIS TEXT IS AUTOGENERATED. DO NOT EDIT BY HAND. -->
<!-- see templates/registry/markdown/snippet.md.j2 -->
<!-- prettier-ignore-start -->
...
| Name | Instrument Type | Unit (UCUM) | Description | Stability | Entity Associations |
| -------- | --------------- | ----------- | -------------- | --------- | ------ |
| `gen_ai.invoke_workflow.inference_calls` | Histogram | `{inference_call}` | The number of inference (model) calls made during a single GenAI workflow execution. [1] | ![Development](https://img.shields.io/badge/-development-blue) | |

**[1]:** The distribution is scoped to a single workflow execution and SHOULD
include every inference call in the call tree below the corresponding
`gen_ai.invoke_workflow.internal` span including failed ones. Inference
calls made in a nested workflow are also recorded against every
enclosing workflow execution.

Instrumentation SHOULD record the count when the execution ends,
including when it is zero. Instrumentation that cannot observe every
inference call inside the execution SHOULD NOT record this metric.

This metric SHOULD be emitted together with the
`gen_ai.invoke_workflow.internal` span for the same execution.
...
| Key | Stability | [Requirement Level](https://opentelemetry.io/docs/specs/semconv/general/attribute-requirement-level/) | Value Type | Description | Example Values |
| --- | --- | --- | --- | --- | --- |
| [`gen_ai.workflow.name`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` | string | Human-readable name of the GenAI workflow provided by the application. [1] | `multi_agent_rag`; `customer_support_pipeline` |
...
### Metric: `gen_ai.invoke_workflow.tool_calls`

This metric is [recommended][MetricRecommended] for instrumentations of agent
frameworks that can reliably attribute tool calls to a single workflow
execution.

This metric SHOULD be specified with [ExplicitBucketBoundaries] of
[0, 1, 2, 4, 8, 16, 32, 64, 128, 256, 512].

<!-- weaver .registry.metrics[] | select(.name == "gen_ai.invoke_workflow.tool_calls") -->
<!-- NOTE: THIS TEXT IS AUTOGENERATED. DO NOT EDIT BY HAND. -->
<!-- see templates/registry/markdown/snippet.md.j2 -->
<!-- prettier-ignore-start -->
...
| Name | Instrument Type | Unit (UCUM) | Description | Stability | Entity Associations |
| -------- | --------------- | ----------- | -------------- | --------- | ------ |
| `gen_ai.invoke_workflow.tool_calls` | Histogram | `{tool_call}` | The number of tool calls made during a single GenAI workflow execution. [1] | ![Development](https://img.shields.io/badge/-development-blue) | |

**[1]:** The distribution is scoped to a single workflow execution and SHOULD
include every tool call in the call tree below the corresponding
`gen_ai.invoke_workflow.internal` span including failed ones. Calls that
route control between agents, such as a handoff or a `transfer_to_agent`
call, count as tool calls. Tool calls made in a nested workflow are also
recorded against every enclosing workflow execution.
...
Instrumentation SHOULD record the count when the execution ends,
including when it is zero. Instrumentation that cannot observe every
tool call inside the execution SHOULD NOT record this metric.
...
[MetricRecommended]: https://github.com/open-telemetry/semantic-conventions/blob/v1.44.0/docs/general/signal-requirement-level.md
