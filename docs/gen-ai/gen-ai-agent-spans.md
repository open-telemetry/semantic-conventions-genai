...
| [`gen_ai.agent.finish_reason`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` [4] | string | The reason an agent invocation loop terminated. [5] | `completed`; `guardrail`; `max_iterations` |
| [`gen_ai.agent.id`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` If applicable. | string | The unique identifier of the GenAI agent. | `asst_5j66UpCpwteGg4YSxUnt7lPY` |
...
| [`gen_ai.conversation.id`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` When available. | string | The unique identifier for a conversation (session, thread), used to store and correlate messages within this conversation. [6] | `conv_5j66UpCpwteGg4YSxUnt7lPY` |
...
**[4] `gen_ai.agent.finish_reason`:** When the agent loop has terminated and the termination reason is known.

**[5] `gen_ai.agent.finish_reason`:** This is the agent-loop equivalent of `gen_ai.response.finish_reasons`, which
records why a single LLM call stopped generating. `gen_ai.agent.finish_reason`
records why the surrounding agent loop exited.

It MUST only be set when the agent invocation has terminated; in-flight spans
MUST NOT set it. Instrumentations MAY emit framework-specific values not in
the list above when no listed value applies.

Detection is framework-specific. For example, instrumentations can set:

- `completed` when the framework reaches its normal exit path.
- `guardrail` when the framework terminates because a guardrail or safety
  policy stops the agent invocation.
- `max_iterations` when the framework terminates because an iteration or
...
`gen_ai.agent.finish_reason` has the following list of well-known values. If one of them applies, then the respective value MUST be used; otherwise, a custom value MAY be used.

| Value | Description | Stability |
| --- | --- | --- |
| `completed` | The agent loop terminated normally. | ![Development](https://img.shields.io/badge/-development-blue) |
| `guardrail` | The agent loop terminated because a guardrail or safety policy stopped the invocation. | ![Development](https://img.shields.io/badge/-development-blue) |
| `max_iterations` | The agent loop terminated because an implementation-defined iteration (turn) limit was reached. | ![Development](https://img.shields.io/badge/-development-blue) |
