...
**[9]:** The fetch itself consumes no tokens and instrumentations SHOULD NOT report token usage metrics for this operation. Token usage carried on the fetched response describes the original generation. The `fetch_response` span defines how it is recorded.
...
**[10]:** The fetch itself consumes no tokens and instrumentations SHOULD NOT report token usage metrics for this operation. Token usage carried on the fetched response describes the original generation. The `fetch_response` span defines how it is recorded.
...
The fetch consumes no tokens, and token usage metrics are not reported for this operation.
Token usage carried on the fetched response describes the generation identified by
`gen_ai.response.id`. The same values may also be present on the inference span that
started that generation and on other `fetch_response` spans for the same identifier, so
`gen_ai.usage.*` attributes on this span MUST NOT be summed across spans. Consumers SHOULD
keep one value per `gen_ai.response.id`.

For background execution, where the client polls with `fetch_response` until the response
reaches a terminal status, the fetch that observes the terminal status is the first place
the client sees usage. A fetch of a response that is still `queued` or `in_progress`
carries no usage.
...
| [`gen_ai.usage.input_tokens`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` If the fetched response carries token usage. | int | The number of tokens used in the GenAI input (prompt). [10] | `100` |
| [`gen_ai.usage.output_tokens`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` If the fetched response carries token usage. | int | The number of tokens used in the GenAI response (completion). [11] | `180` |
| [`server.address`](https://github.com/open-telemetry/semantic-conventions/blob/v1.44.0/docs/registry/attributes/server.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Recommended` | string | GenAI server address. [12] | `example.com`; `10.1.2.80`; `/tmp/my.sock` |
...
**[10] `gen_ai.usage.input_tokens`:** The input token count of the generation identified by `gen_ai.response.id`, as carried on the fetched response. A snapshot of that generation, not additive across spans.

**[11] `gen_ai.usage.output_tokens`:** The output token count of the generation identified by `gen_ai.response.id`, as carried on the fetched response. A snapshot of that generation, not additive across spans.
...
**[14] `gen_ai.system_instructions`:** Only content carried on the fetched response is recorded. A fetched response contains the system instructions, output messages, and tool definitions but not the original input messages, so `gen_ai.input.messages` is not set on this span.
...
**[16]:** The fetch itself consumes no tokens and instrumentations SHOULD NOT report token usage metrics for this operation. Token usage carried on the fetched response describes the original generation. The `fetch_response` span defines how it is recorded.
...
**[13]:** The fetch itself consumes no tokens and instrumentations SHOULD NOT report token usage metrics for this operation. Token usage carried on the fetched response describes the original generation. The `fetch_response` span defines how it is recorded.
