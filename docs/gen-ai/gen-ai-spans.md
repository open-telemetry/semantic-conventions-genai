...
- **User-side span.** Whether to emit a separate span for the user's
  "recording interval" when user voice-activity events are available (for
  example OpenAI Realtime `input_audio_buffer.speech_started`/`speech_stopped`,
  or Gemini Enterprise/Vertex `voiceActivity`) is undecided. Providers without
  user activity events (such as the Gemini Developer API) could not emit it.
- **User transcript placement.** When a user-audio transcript is available, it
  is unclear whether it belongs on the `gen_ai.generate_live_content.client`
  input messages, on a user-side span, or both.
