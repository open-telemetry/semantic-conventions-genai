...
- [Spans](#spans)
  ...
  - [Live content generation](#live-content-generation)
    ...
    - [User input span](#user-input-span)
...
| [`gen_ai.usage.audio.input_tokens`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` [16] | int | The number of audio tokens used in the GenAI input (prompt). [17] | `50` |
| [`gen_ai.usage.audio.output_tokens`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` [18] | int | The number of audio tokens used in the GenAI response (completion). [19] | `30` |
| [`gen_ai.usage.cache_creation.input_tokens`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` | int | The number of input tokens written to a provider-managed cache. [20] | `25` |
| [`gen_ai.usage.cache_read.input_tokens`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` | int | The number of input tokens served from a provider-managed cache. [21] | `50` |
...
| [`gen_ai.prompt.variable`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Opt-In` | string | The variables supplied to the prompt template, the `<key>` being the variable name, the value being the variable value. [28] | `Alice`; `French` |
...
**[16] `gen_ai.usage.audio.input_tokens`:** When the model reports audio input tokens separately.

**[17] `gen_ai.usage.audio.input_tokens`:** The value SHOULD be included in `gen_ai.usage.input_tokens`.
Voice-native (speech-to-speech) models and multimodal models that accept audio input report audio tokens separately from text tokens. When the provider exposes an audio token count for the input (for example, OpenAI `usage.prompt_tokens_details.audio_tokens` or the Realtime API `usage.input_token_details.audio_tokens`), instrumentations SHOULD report it here.

**[18] `gen_ai.usage.audio.output_tokens`:** When the model reports audio output tokens separately.

**[19] `gen_ai.usage.audio.output_tokens`:** The value SHOULD be included in `gen_ai.usage.output_tokens`.
Voice-native (speech-to-speech) models and multimodal models that produce audio output report audio tokens separately from text tokens. When the provider exposes an audio token count for the output (for example, OpenAI `usage.completion_tokens_details.audio_tokens` or the Realtime API `usage.output_token_details.audio_tokens`), instrumentations SHOULD report it here.
...
**[30] `gen_ai.tool.definitions`:** Since this attribute could be large, it's NOT RECOMMENDED to populate
...
| `user_input` | Client-side capture of a user input (utterance) within a live, bidirectional speech-to-speech session, delimited by the provider's user voice-activity (start/stop) events. [32] | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[32]:** This operation is optional and provider-dependent. It is recorded only when the provider exposes user voice-activity detection (VAD) events that bracket when the user started and stopped speaking (for example OpenAI Realtime `input_audio_buffer.speech_started` / `input_audio_buffer.speech_stopped`). Providers without user VAD events (such as the Gemini Developer API) cannot record it; the user input is then carried on the `generate_live_content` operation through `gen_ai.input.messages` instead.
...
Live, bidirectional speech-to-speech APIs (such as
...
cannot be represented by the [inference](#inference) span, whose duration is the
client-observed request/response.

Instead, each server-side generation is represented by a
`gen_ai.generate_live_content.client` span, reconstructed from the provider's
...
one) and end at the generation completion or interruption event. If usage is
reported after the completion event, instrumentation SHOULD wait for it before
ending the span.
...
through `gen_ai.conversation.id`. Turn-level spans (a full user-and-model
...
[event] gen_ai.client.live_session.started
        user_input                             (span, optional — only with user voice-activity events)
...
flowchart TD
    ...
    U1["span: user_input (optional)"]
    S1["span: generate_live_content {model}"]
...
flowchart TD
    subgraph sibling["Sibling pattern (OpenAI Realtime, Gemini Live 2.5)"]
        direction TB
        A1["span: generate_live_content {model} (call)"]
        A2["span: execute_tool {tool}"]
        A3["span: generate_live_content {model} (answer)"]
        A1 --> A2 --> A3
    end

    subgraph child["Child pattern (Gemini Live 3.x)"]
        direction TB
        subgraph B1["span: generate_live_content {model}"]
...
<!-- weaver .registry.spans[] | select(.type == "gen_ai.generate_live_content.client") -->
<!-- NOTE: THIS TEXT IS AUTOGENERATED. DO NOT EDIT BY HAND. -->
<!-- see templates/registry/markdown/snippet.md.j2 -->
<!-- prettier-ignore-start -->
...
Describes one server-side model generation within a live, bidirectional speech-to-speech session (such as OpenAI Realtime or Gemini Live).

Unlike `gen_ai.inference.client`, this span is not a single client-timed request/response.
The client streams user audio continuously and the server streams incremental events; the
generation is reconstructed from those events. The span SHOULD start at the first output
chunk of the generation (or at an earlier voice-activity end anchor when the provider
exposes one) and end at the generation completion or interruption event. If usage is
reported after the completion event, instrumentation SHOULD wait for it before ending the
span.

The `gen_ai.operation.name` SHOULD be `generate_live_content`.
...
`gen_ai.conversation.id` correlating the generations within it. Turn-level spans (a full
...
- Sibling pattern (OpenAI Realtime, Gemini Live 2.5): the provider closes the generation,
  ...
  sibling `generate_live_content` spans with the `execute_tool` span between them, so
...
When the provider exposes user voice-activity events, the user input MAY additionally be
modeled by an optional `gen_ai.user_input.client` span; otherwise the user input is carried
...
| [`gen_ai.response.finish_reasons`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` | string[] | Array of reasons the model stopped generating tokens, corresponding to each generation received. | `["stop"]`; `["stop", "length"]` |
| [`gen_ai.response.id`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` [11] | string | The unique identifier for the completion. | `chatcmpl-123` |
...
| [`gen_ai.response.time_to_first_chunk`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` | double | Time to first chunk in a streaming response, measured from request issuance, in seconds. The value is measured from when the client issues the generation request to when the first chunk is received in the response stream. | `0.5`; `1.2` |
| [`gen_ai.usage.audio.input_tokens`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` [13] | int | The number of audio tokens used in the GenAI input (prompt). [14] | `50` |
| [`gen_ai.usage.audio.output_tokens`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` [15] | int | The number of audio tokens used in the GenAI response (completion). [16] | `30` |
...
**[5] `gen_ai.conversation.id`:** For a live session this correlates the generations that belong to the same session. It is a within-session correlation key, not necessarily a durable, retrievable resource.
...
**[11] `gen_ai.response.id`:** When available. A live generation may not carry a response id on all providers.

**[12] `gen_ai.response.model`:** If available. The name of the GenAI model that provided the response. If the model is supplied by a vendor, then the value must be the exact name of the model actually used. If the model is a fine-tuned custom model, the value should have a more specific name than the base model that's been fine-tuned.

**[13] `gen_ai.usage.audio.input_tokens`:** When the model reports audio input tokens separately.

**[14] `gen_ai.usage.audio.input_tokens`:** The value SHOULD be included in `gen_ai.usage.input_tokens`.
Voice-native (speech-to-speech) models and multimodal models that accept audio input report audio tokens separately from text tokens. When the provider exposes an audio token count for the input (for example, OpenAI `usage.prompt_tokens_details.audio_tokens` or the Realtime API `usage.input_token_details.audio_tokens`), instrumentations SHOULD report it here.

**[15] `gen_ai.usage.audio.output_tokens`:** When the model reports audio output tokens separately.
...
| `execute_tool` | Execute a tool | ![Development](https://img.shields.io/badge/-development-blue) |
| `generate_content` | Multimodal content generation operation such as [Gemini Generate Content](https://ai.google.dev/api/generate-content) | ![Development](https://img.shields.io/badge/-development-blue) |
| `generate_live_content` | Server-side model generation within a live, bidirectional speech-to-speech session such as [OpenAI Realtime](https://platform.openai.com/docs/guides/realtime) or [Gemini Live](https://ai.google.dev/gemini-api/docs/live). [27] | ![Development](https://img.shields.io/badge/-development-blue) |
| `invoke_agent` | Invoke GenAI agent | ![Development](https://img.shields.io/badge/-development-blue) |
| `invoke_workflow` | Invoke GenAI workflow | ![Development](https://img.shields.io/badge/-development-blue) |
| `plan` | Agent planning or task decomposition phase | ![Development](https://img.shields.io/badge/-development-blue) |
| `retrieval` | Retrieval operation such as [OpenAI Search Vector Store API](https://platform.openai.com/docs/api-reference/vector-stores/search) | ![Development](https://img.shields.io/badge/-development-blue) |
...
| `update_memory` | Update existing memory records | ![Development](https://img.shields.io/badge/-development-blue) |
| `upsert_memory` | Create or update memory records without the caller choosing which | ![Development](https://img.shields.io/badge/-development-blue) |
| `user_input` | Client-side capture of a user input (utterance) within a live, bidirectional speech-to-speech session, delimited by the provider's user voice-activity (start/stop) events. [28] | ![Development](https://img.shields.io/badge/-development-blue) |

**[27]:** This operation describes a single server-side generation reconstructed from a long-lived streaming session, not a single client request/response. It is distinct from `chat` and `generate_content`: the request is streamed continuously and the generation is bounded by provider events (first output chunk or a voice-activity end anchor through generation completion or interruption) rather than by a client call boundary.

**[28]:** This operation is optional and provider-dependent. It is recorded only when the provider exposes user voice-activity detection (VAD) events that bracket when the user started and stopped speaking (for example OpenAI Realtime `input_audio_buffer.speech_started` / `input_audio_buffer.speech_stopped`). Providers without user VAD events (such as the Gemini Developer API) cannot record it; the user input is then carried on the `generate_live_content` operation through `gen_ai.input.messages` instead.
...
emit an optional `gen_ai.user_input.client` span for the user's speaking
...
following `gen_ai.generate_live_content.client` span through
`gen_ai.input.messages`.

<!-- weaver .registry.spans[] | select(.type == "gen_ai.user_input.client") -->
<!-- NOTE: THIS TEXT IS AUTOGENERATED. DO NOT EDIT BY HAND. -->
<!-- see templates/registry/markdown/snippet.md.j2 -->
<!-- prettier-ignore-start -->
...
Describes the client-side capture of a user input (utterance) within a live, bidirectional speech-to-speech session, delimited by the provider's user voice-activity (start/stop) events.
...
`gen_ai.generate_live_content.client` span through `gen_ai.input.messages` instead.
...
**[5] `gen_ai.conversation.id`:** For a live session this correlates the user input with the generations that respond to it.
...
**[9] `gen_ai.input.messages`:** When content capture is enabled and a transcript of the user's utterance is available, it is carried here as the user input message.
...
| `execute_tool` | Execute a tool | ![Development](https://img.shields.io/badge/-development-blue) |
| `generate_content` | Multimodal content generation operation such as [Gemini Generate Content](https://ai.google.dev/api/generate-content) | ![Development](https://img.shields.io/badge/-development-blue) |
| `generate_live_content` | Server-side model generation within a live, bidirectional speech-to-speech session such as [OpenAI Realtime](https://platform.openai.com/docs/guides/realtime) or [Gemini Live](https://ai.google.dev/gemini-api/docs/live). [10] | ![Development](https://img.shields.io/badge/-development-blue) |
| `invoke_agent` | Invoke GenAI agent | ![Development](https://img.shields.io/badge/-development-blue) |
| `invoke_workflow` | Invoke GenAI workflow | ![Development](https://img.shields.io/badge/-development-blue) |
| `plan` | Agent planning or task decomposition phase | ![Development](https://img.shields.io/badge/-development-blue) |
| `retrieval` | Retrieval operation such as [OpenAI Search Vector Store API](https://platform.openai.com/docs/api-reference/vector-stores/search) | ![Development](https://img.shields.io/badge/-development-blue) |
...
| `update_memory` | Update existing memory records | ![Development](https://img.shields.io/badge/-development-blue) |
| `upsert_memory` | Create or update memory records without the caller choosing which | ![Development](https://img.shields.io/badge/-development-blue) |
| `user_input` | Client-side capture of a user input (utterance) within a live, bidirectional speech-to-speech session, delimited by the provider's user voice-activity (start/stop) events. [11] | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[11]:** This operation is optional and provider-dependent. It is recorded only when the provider exposes user voice-activity detection (VAD) events that bracket when the user started and stopped speaking (for example OpenAI Realtime `input_audio_buffer.speech_started` / `input_audio_buffer.speech_stopped`). Providers without user VAD events (such as the Gemini Developer API) cannot record it; the user input is then carried on the `generate_live_content` operation through `gen_ai.input.messages` instead.
...
| `execute_tool` | Execute a tool | ![Development](https://img.shields.io/badge/-development-blue) |
| `generate_content` | Multimodal content generation operation such as [Gemini Generate Content](https://ai.google.dev/api/generate-content) | ![Development](https://img.shields.io/badge/-development-blue) |
| `generate_live_content` | Server-side model generation within a live, bidirectional speech-to-speech session such as [OpenAI Realtime](https://platform.openai.com/docs/guides/realtime) or [Gemini Live](https://ai.google.dev/gemini-api/docs/live). [9] | ![Development](https://img.shields.io/badge/-development-blue) |
| `invoke_agent` | Invoke GenAI agent | ![Development](https://img.shields.io/badge/-development-blue) |
| `invoke_workflow` | Invoke GenAI workflow | ![Development](https://img.shields.io/badge/-development-blue) |
| `plan` | Agent planning or task decomposition phase | ![Development](https://img.shields.io/badge/-development-blue) |
| `retrieval` | Retrieval operation such as [OpenAI Search Vector Store API](https://platform.openai.com/docs/api-reference/vector-stores/search) | ![Development](https://img.shields.io/badge/-development-blue) |
...
| `update_memory` | Update existing memory records | ![Development](https://img.shields.io/badge/-development-blue) |
| `upsert_memory` | Create or update memory records without the caller choosing which | ![Development](https://img.shields.io/badge/-development-blue) |
| `user_input` | Client-side capture of a user input (utterance) within a live, bidirectional speech-to-speech session, delimited by the provider's user voice-activity (start/stop) events. [10] | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[10]:** This operation is optional and provider-dependent. It is recorded only when the provider exposes user voice-activity detection (VAD) events that bracket when the user started and stopped speaking (for example OpenAI Realtime `input_audio_buffer.speech_started` / `input_audio_buffer.speech_stopped`). Providers without user VAD events (such as the Gemini Developer API) cannot record it; the user input is then carried on the `generate_live_content` operation through `gen_ai.input.messages` instead.
...
| `execute_tool` | Execute a tool | ![Development](https://img.shields.io/badge/-development-blue) |
| `generate_content` | Multimodal content generation operation such as [Gemini Generate Content](https://ai.google.dev/api/generate-content) | ![Development](https://img.shields.io/badge/-development-blue) |
| `generate_live_content` | Server-side model generation within a live, bidirectional speech-to-speech session such as [OpenAI Realtime](https://platform.openai.com/docs/guides/realtime) or [Gemini Live](https://ai.google.dev/gemini-api/docs/live). [13] | ![Development](https://img.shields.io/badge/-development-blue) |
| `invoke_agent` | Invoke GenAI agent | ![Development](https://img.shields.io/badge/-development-blue) |
| `invoke_workflow` | Invoke GenAI workflow | ![Development](https://img.shields.io/badge/-development-blue) |
| `plan` | Agent planning or task decomposition phase | ![Development](https://img.shields.io/badge/-development-blue) |
| `retrieval` | Retrieval operation such as [OpenAI Search Vector Store API](https://platform.openai.com/docs/api-reference/vector-stores/search) | ![Development](https://img.shields.io/badge/-development-blue) |
...
| `update_memory` | Update existing memory records | ![Development](https://img.shields.io/badge/-development-blue) |
| `upsert_memory` | Create or update memory records without the caller choosing which | ![Development](https://img.shields.io/badge/-development-blue) |
| `user_input` | Client-side capture of a user input (utterance) within a live, bidirectional speech-to-speech session, delimited by the provider's user voice-activity (start/stop) events. [14] | ![Development](https://img.shields.io/badge/-development-blue) |

**[13]:** This operation describes a single server-side generation reconstructed from a long-lived streaming session, not a single client request/response. It is distinct from `chat` and `generate_content`: the request is streamed continuously and the generation is bounded by provider events (first output chunk or a voice-activity end anchor through generation completion or interruption) rather than by a client call boundary.

**[14]:** This operation is optional and provider-dependent. It is recorded only when the provider exposes user voice-activity detection (VAD) events that bracket when the user started and stopped speaking (for example OpenAI Realtime `input_audio_buffer.speech_started` / `input_audio_buffer.speech_stopped`). Providers without user VAD events (such as the Gemini Developer API) cannot record it; the user input is then carried on the `generate_live_content` operation through `gen_ai.input.messages` instead.
...
| `execute_tool` | Execute a tool | ![Development](https://img.shields.io/badge/-development-blue) |
| `generate_content` | Multimodal content generation operation such as [Gemini Generate Content](https://ai.google.dev/api/generate-content) | ![Development](https://img.shields.io/badge/-development-blue) |
| `generate_live_content` | Server-side model generation within a live, bidirectional speech-to-speech session such as [OpenAI Realtime](https://platform.openai.com/docs/guides/realtime) or [Gemini Live](https://ai.google.dev/gemini-api/docs/live). [6] | ![Development](https://img.shields.io/badge/-development-blue) |
| `invoke_agent` | Invoke GenAI agent | ![Development](https://img.shields.io/badge/-development-blue) |
| `invoke_workflow` | Invoke GenAI workflow | ![Development](https://img.shields.io/badge/-development-blue) |
| `plan` | Agent planning or task decomposition phase | ![Development](https://img.shields.io/badge/-development-blue) |
| `retrieval` | Retrieval operation such as [OpenAI Search Vector Store API](https://platform.openai.com/docs/api-reference/vector-stores/search) | ![Development](https://img.shields.io/badge/-development-blue) |
...
| `update_memory` | Update existing memory records | ![Development](https://img.shields.io/badge/-development-blue) |
| `upsert_memory` | Create or update memory records without the caller choosing which | ![Development](https://img.shields.io/badge/-development-blue) |
| `user_input` | Client-side capture of a user input (utterance) within a live, bidirectional speech-to-speech session, delimited by the provider's user voice-activity (start/stop) events. [7] | ![Development](https://img.shields.io/badge/-development-blue) |

**[6]:** This operation describes a single server-side generation reconstructed from a long-lived streaming session, not a single client request/response. It is distinct from `chat` and `generate_content`: the request is streamed continuously and the generation is bounded by provider events (first output chunk or a voice-activity end anchor through generation completion or interruption) rather than by a client call boundary.

**[7]:** This operation is optional and provider-dependent. It is recorded only when the provider exposes user voice-activity detection (VAD) events that bracket when the user started and stopped speaking (for example OpenAI Realtime `input_audio_buffer.speech_started` / `input_audio_buffer.speech_stopped`). Providers without user VAD events (such as the Gemini Developer API) cannot record it; the user input is then carried on the `generate_live_content` operation through `gen_ai.input.messages` instead.
