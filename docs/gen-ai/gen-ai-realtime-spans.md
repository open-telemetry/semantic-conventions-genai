...
- [Realtime content generation](#realtime-content-generation)
...
realtime, bidirectional streaming sessions (such as OpenAI Realtime or Gemini
Live), where there is no single request/response call boundary.

## Realtime content generation

Realtime, bidirectional streaming APIs (such as
...
The realtime session itself is long-lived and mostly idle, so it is not modeled as a
span. It is represented by the
[`gen_ai.client.realtime_session.started`](gen-ai-events.md#event-gen_aiclientrealtime_sessionstarted)
and
[`gen_ai.client.realtime_session.ended`](gen-ai-events.md#event-gen_aiclientrealtime_sessionended)
events, and the generations that belong to the same session are correlated
through `gen_ai.realtime_session.id`. Turn-level spans (a full user-and-model
exchange) are out of scope: turn boundaries cannot be detected reliably across
providers.

### Trace structure

A realtime session is bracketed by the `started` / `ended` events (not a span), and
each server-side generation is a `gen_ai.realtime_inference.client` span
correlated by `gen_ai.realtime_session.id`. The `gen_ai.realtime_session.id` is the
...
`gen_ai.realtime_session.id`. `gen_ai.realtime_session.id` is orthogonal to
`gen_ai.conversation.id`: a single conversation can span multiple realtime sessions.
...
```text
[event] gen_ai.client.realtime_session.started
        user_speech                            (span, optional — only with user voice-activity events)
        realtime_inference {model}          (span, correlated by gen_ai.realtime_session.id)
[event] gen_ai.client.realtime_session.ended
```

```mermaid
flowchart TD
    E0["event: gen_ai.client.realtime_session.started"]
    U1["span: user_speech (optional)"]
    S1["span: realtime_inference {model}"]
    E1["event: gen_ai.client.realtime_session.ended"]
    E0 -.-> U1 -.-> S1 -.-> E1
```

When a generation calls a tool, the realtime API returns the tool call to the
client, which runs the tool and sends the result back to the model. This
client-side tool execution is application code, not a model operation, so a
realtime generation does not emit an `execute_tool` span. The tool exchange is
captured as message parts instead: the model's request as a `tool_call` output
part and the client's result as a `tool_call_response` input part.

How those parts are distributed across generations is provider-dependent. Some
...
a single generation open across the tool round-trip (**child pattern** — Gemini
Live 3.x):
...
realtime_inference {model} (call)        realtime_inference {model}
  └─ output: tool_call part                (tool round-trip resolved inside;
realtime_inference {model} (answer)         no separate tool_call parts)
  └─ input: tool_call_response part
```
...
flowchart TD
    subgraph sibling["Sibling pattern (OpenAI Realtime, Gemini Live 2.5)"]
        direction TB
        A1["span: realtime_inference {model} (call) — tool_call output part"]
        A3["span: realtime_inference {model} (answer) — tool_call_response input part"]
        A1 --> A3
    end

    subgraph child["Child pattern (Gemini Live 3.x)"]
        direction TB
        B1["span: realtime_inference {model} (tool round-trip resolved inside)"]
    end
```

In the sibling pattern the tool request and result appear on two consecutive
generations. In the child pattern a single generation stays open across the tool
round-trip, so the intermediate tool exchange is not surfaced as separate message
parts.
...
Describes one server-side model generation within a realtime, bidirectional streaming session (such as OpenAI Realtime or Gemini Live).

Unlike `gen_ai.inference.client`, this span is not a single client-timed request/response.
A *generation* is one server-side model response within the realtime session (the model's
...
Instrumentation SHOULD NOT create this span by default when the expected generation
duration can be long (for example minutes or more), such as speech translation or
transcription or other cases where there is no conversation between the user and the model.
Instrumentations MAY use the model name or other heuristics available in the instrumented
library to determine whether the call is expected to be conversational.

The `gen_ai.operation.name` SHOULD be `realtime_inference`.

The realtime session itself is not modeled as a span. It is represented by the
`gen_ai.client.realtime_session.started` and `gen_ai.client.realtime_session.ended` events and by
`gen_ai.realtime_session.id` correlating the generations within it. Turn-level spans (a full
...
| [`gen_ai.realtime_session.id`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` [8] | string | The unique identifier for a realtime, bidirectional streaming session. [9] | `sess_5j66UpCpwteGg4YSxUnt7lPY` |
...
| [`gen_ai.response.finish_reasons`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` | string[] | Array of reasons the model stopped generating tokens, corresponding to each generation received. [13] | `["stop"]`; `["stop", "length"]`; `["stop", "length", "error"]` |
| [`gen_ai.response.id`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` [14] | string | The unique identifier for the completion. | `chatcmpl-123` |
...
| [`gen_ai.usage.reasoning.output_tokens`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` When applicable. | int | The number of output tokens used for reasoning (e.g. chain-of-thought, extended thinking). [26] | `50` |
...
**[12] `gen_ai.request.reasoning.level`:** The value SHOULD be the exact string value sent to the provider.
Semantic conventions for individual providers SHOULD document which input parameter maps to this attribute.

**[13] `gen_ai.response.finish_reasons`:** Values correspond to generations in the same order as the returned
choices/candidates.
...
**[14] `gen_ai.response.id`:** When available. A realtime generation may not carry a response id on all providers.
...
**[26] `gen_ai.usage.reasoning.output_tokens`:** The value SHOULD be included in `gen_ai.usage.output_tokens`.
...
| `realtime_inference` | Server-side model generation within a realtime, bidirectional streaming session such as [OpenAI Realtime](https://platform.openai.com/docs/guides/realtime) or [Gemini Live](https://ai.google.dev/gemini-api/docs/live). [36] | ![Development](https://img.shields.io/badge/-development-blue) |
...
| `gcp.gemini` | [Gemini](https://cloud.google.com/products/gemini) [38] | ![Development](https://img.shields.io/badge/-development-blue) |
| `gcp.gen_ai` | Any Google generative AI endpoint [39] | ![Development](https://img.shields.io/badge/-development-blue) |
| `gcp.vertex_ai` | [Vertex AI](https://cloud.google.com/vertex-ai) [40] | ![Development](https://img.shields.io/badge/-development-blue) |
...
**[38]:** Used when accessing the 'generativelanguage.googleapis.com' endpoint. Also known as the AI Studio API.

**[39]:** May be used when specific backend is unknown.

**[40]:** Used when accessing the 'aiplatform.googleapis.com' endpoint.
...
| `update_memory` | Update existing memory records | ![Development](https://img.shields.io/badge/-development-blue) |
| `upsert_memory` | Create or update memory records without the caller choosing which | ![Development](https://img.shields.io/badge/-development-blue) |
| `user_speech` | Client-side capture of a user speech utterance within a realtime, bidirectional speech-to-speech session, delimited by the provider's user voice-activity (start/stop) events. [9] | ![Development](https://img.shields.io/badge/-development-blue) |
...
- **Session modeling.** Will a session also get a long-lived session or conversation span parenting all generations, or stay events + `gen_ai.realtime_session.id` only?
...
- **Tool-call representation.** The client runs the tool (not the realtime
  instrumentation), so the tool exchange is captured as `tool_call` /
  `tool_call_response` message parts rather than an `execute_tool` span. The
  sibling pattern (OpenAI Realtime, Gemini Live 2.5) carries them on two
  consecutive generations; the child pattern (Gemini Live 3.x) resolves the tool
  round-trip inside one generation and does not surface the parts. A single
  normative recommendation across providers is still open.
