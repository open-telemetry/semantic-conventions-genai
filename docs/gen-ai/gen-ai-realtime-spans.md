...
| [`gen_ai.realtime_session.id`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` [6] | string | The unique identifier for a realtime, bidirectional streaming session. [7] | `sess_5j66UpCpwteGg4YSxUnt7lPY` |
...
**[6] `gen_ai.realtime_session.id`:** If the provider exposes a session identifier, or the instrumentation uses a client-created per-connection identifier.

**[7] `gen_ai.realtime_session.id`:** A realtime session is a single long-lived streaming connection during which the client
...
**[12] `gen_ai.response.id`:** When available. A realtime generation may not carry a response id on all providers.
...
| `user_speech` | Client-side capture of a user speech utterance within a realtime, bidirectional speech-to-speech session, delimited by the provider's user voice-activity (start/stop) events. [35] | ![Development](https://img.shields.io/badge/-development-blue) |
