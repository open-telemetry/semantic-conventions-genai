...
| <a id="gen-ai-evaluation-evaluator-id" href="#gen-ai-evaluation-evaluator-id">`gen_ai.evaluation.evaluator.id`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | Stable identifier for the evaluator definition, not the invocation. [5] | `eval-7f3a9c2d`; `accuracy-evaluator` |
| <a id="gen-ai-evaluation-evaluator-type" href="#gen-ai-evaluation-evaluator-type">`gen_ai.evaluation.evaluator.type`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | The mechanism that determines the evaluation result. [6] | `llm_judge`; `deterministic`; `human` |
...
| <a id="gen-ai-evaluation-reference-set-id" href="#gen-ai-evaluation-reference-set-id">`gen_ai.evaluation.reference_set.id`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | Stable identifier for the reference set against which the evaluation result was produced. [8] | `dataset-123`; `golden-set-v1` |
...
**[8] `gen_ai.evaluation.reference_set.id`:** Instrumentations SHOULD use a stable identifier provided by the evaluation framework or application when available. The identifier SHOULD distinguish score-affecting revisions of the reference set. The attribute SHOULD be omitted when no stable reference set identifier is available.
...
**[40] `gen_ai.workflow.name`:** The workflow name is usually a static, application-unique identifier defined
