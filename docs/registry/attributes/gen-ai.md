...
| <a id="gen-ai-evaluation-evaluator-id" href="#gen-ai-evaluation-evaluator-id">`gen_ai.evaluation.evaluator.id`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | Stable identifier for the evaluator definition, not the invocation. Results with different evaluator ids must not be treated as comparable. [5] | `judge-v1`; `accuracy-model-abc` |
| <a id="gen-ai-evaluation-evaluator-type" href="#gen-ai-evaluation-evaluator-type">`gen_ai.evaluation.evaluator.type`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | The type of evaluator. [6] | `llm_judge`; `deterministic`; `human` |
| <a id="gen-ai-evaluation-evaluator-version" href="#gen-ai-evaluation-evaluator-version">`gen_ai.evaluation.evaluator.version`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | Version of the evaluator definition. [7] | `1.0`; `v2.5.1` |
...
| <a id="gen-ai-evaluation-reference-set-id" href="#gen-ai-evaluation-reference-set-id">`gen_ai.evaluation.reference_set.id`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | Identifier of the golden/reference set against which the score was produced. [8] | `dataset-123`; `golden-set-v1` |
| <a id="gen-ai-evaluation-scope" href="#gen-ai-evaluation-scope">`gen_ai.evaluation.scope`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | The unit the result is about. [9] | `single_output`; `full_run`; `human_interaction` |
...
| <a id="gen-ai-usage-output-tokens" href="#gen-ai-usage-output-tokens">`gen_ai.usage.output_tokens`</a> | ![Development](https://img.shields.io/badge/-development-blue) | int | The number of tokens used in the GenAI response (completion). [39] | `180` |
| <a id="gen-ai-usage-reasoning-output-tokens" href="#gen-ai-usage-reasoning-output-tokens">`gen_ai.usage.reasoning.output_tokens`</a> | ![Development](https://img.shields.io/badge/-development-blue) | int | The number of output tokens used for reasoning (e.g. chain-of-thought, extended thinking). [40] | `50` |
| <a id="gen-ai-workflow-name" href="#gen-ai-workflow-name">`gen_ai.workflow.name`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | Human-readable name of the GenAI workflow provided by the application. [41] | `multi_agent_rag`; `customer_support_pipeline` |
...
**[5] `gen_ai.evaluation.evaluator.id`:** The evaluator id should be a stable identifier for the evaluator definition. To maintain valid comparability, results SHOULD be grouped by `(evaluator.id, evaluator.version)` to prevent invalid aggregations when querying the data.

**[6] `gen_ai.evaluation.evaluator.type`:** `gen_ai.evaluation.evaluator.type` describes who or what produced the score (e.g., a human or an LLM). This is independent of `gen_ai.evaluation.scope`, which describes the unit of the evaluation.

**[7] `gen_ai.evaluation.evaluator.version`:** The version MUST change when any score-affecting part of the definition changes (e.g., for an `llm_judge`: the model, rubric, or scoring config). To maintain valid comparability, results SHOULD be grouped by `(evaluator.id, evaluator.version)`.

**[8] `gen_ai.evaluation.reference_set.id`:** Conditionally required when a reference set was used.

**[9] `gen_ai.evaluation.scope`:** `gen_ai.evaluation.scope` describes the unit of the evaluation (e.g., a single output or a human interaction). This is independent of `gen_ai.evaluation.evaluator.type`, which describes who or what produced the score (e.g., a human or an LLM).
...
**[39] `gen_ai.usage.output_tokens`:** When the provider reports both billed token counts and model-consumed
token counts (for example, Cohere exposes both `usage.billed_units` and
`usage.tokens`), instrumentations SHOULD report the billed count so the
value matches the units the customer is charged for.

**[40] `gen_ai.usage.reasoning.output_tokens`:** The value SHOULD be included in `gen_ai.usage.output_tokens`.

**[41] `gen_ai.workflow.name`:** The workflow name is usually a static, application-unique identifier defined
