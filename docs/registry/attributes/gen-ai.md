...
| <a id="gen-ai-agent-identity-id" href="#gen-ai-agent-identity-id">`gen_ai.agent.identity.id`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | Stable identifier of the identity the GenAI agent authenticates and is audited as, independent of the deployed agent resource (`gen_ai.agent.id`) or runtime. [2] | `spiffe://example.org/agent/support-bot`; `arn:aws:bedrock-agentcore:us-east-1:123456789012:workload-identity/directory/default/workload-identity/support-bot` |
| <a id="gen-ai-agent-identity-name" href="#gen-ai-agent-identity-name">`gen_ai.agent.identity.name`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | Human-readable label for the agent's identity, see `gen_ai.agent.identity.id`. | `customer-support-agent` |
...
**[2] `gen_ai.agent.identity.id`:** `gen_ai.agent.id` identifies the deployed agent resource or definition, while this attribute identifies the durable identity the agent acts as. One identity can span many deployed resource versions and runtimes, so the two are distinct. It MAY be a SPIFFE ID where the platform issues one, for example an AWS Bedrock AgentCore workload identity or a Google Vertex AI Agent Identity.
This is an identifier for correlation and audit only. It is not a credential and not an input to an access-control decision.
...
**[20] `gen_ai.retrieval.documents`:** Instrumentations MUST follow [Retrieval documents JSON schema](/docs/gen-ai/gen-ai-retrieval-documents.json).
When the attribute is recorded on events, it MUST be recorded in structured
form. When recorded on spans, it MAY be recorded as a JSON string if structured
format is not supported and SHOULD be recorded in structured form otherwise.
