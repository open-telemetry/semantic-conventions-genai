...
| <a id="gen-ai-agent-iteration-budget-consumed" href="#gen-ai-agent-iteration-budget-consumed">`gen_ai.agent.iteration_budget.consumed`</a> | ![Development](https://img.shields.io/badge/-development-blue) | int | Iterations completed in this agent invocation. [2] | `3`; `8`; `25` |
| <a id="gen-ai-agent-iteration-budget-limit" href="#gen-ai-agent-iteration-budget-limit">`gen_ai.agent.iteration_budget.limit`</a> | ![Development](https://img.shields.io/badge/-development-blue) | int | Maximum iterations configured for this agent invocation. [3] | `10`; `25`; `50` |
...
| <a id="gen-ai-agent-token-budget-consumed" href="#gen-ai-agent-token-budget-consumed">`gen_ai.agent.token_budget.consumed`</a> | ![Development](https://img.shields.io/badge/-development-blue) | int | Cumulative tokens consumed by this agent's own inference calls, excluding sub-agent consumption. [4] | `12340`; `48500`; `99800` |
| <a id="gen-ai-agent-token-budget-limit" href="#gen-ai-agent-token-budget-limit">`gen_ai.agent.token_budget.limit`</a> | ![Development](https://img.shields.io/badge/-development-blue) | int | Cumulative token budget configured for this agent invocation. [5] | `50000`; `100000`; `500000` |
...
| <a id="gen-ai-conversation-compacted" href="#gen-ai-conversation-compacted">`gen_ai.conversation.compacted`</a> | ![Development](https://img.shields.io/badge/-development-blue) | boolean | Indicates whether the effective conversation context used for this operation is a compacted view of a prior conversation. [6] | `true` |
| <a id="gen-ai-conversation-id" href="#gen-ai-conversation-id">`gen_ai.conversation.id`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | The unique identifier for a conversation (session, thread), used to store and correlate messages within this conversation. [7] | `conv_5j66UpCpwteGg4YSxUnt7lPY` |
| <a id="gen-ai-data-source-id" href="#gen-ai-data-source-id">`gen_ai.data_source.id`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | The data source identifier. [8] | `H7STPQYOND` |
...
| <a id="gen-ai-usage-output-tokens" href="#gen-ai-usage-output-tokens">`gen_ai.usage.output_tokens`</a> | ![Development](https://img.shields.io/badge/-development-blue) | int | The number of tokens used in the GenAI response (completion). [38] | `180` |
| <a id="gen-ai-usage-reasoning-output-tokens" href="#gen-ai-usage-reasoning-output-tokens">`gen_ai.usage.reasoning.output_tokens`</a> | ![Development](https://img.shields.io/badge/-development-blue) | int | The number of output tokens used for reasoning (e.g. chain-of-thought, extended thinking). [39] | `50` |
| <a id="gen-ai-workflow-name" href="#gen-ai-workflow-name">`gen_ai.workflow.name`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | Human-readable name of the GenAI workflow provided by the application. [40] | `multi_agent_rag`; `customer_support_pipeline` |
...
**[2] `gen_ai.agent.iteration_budget.consumed`:** Number of iterations the agent completed before the invocation
...
**[3] `gen_ai.agent.iteration_budget.limit`:** The maximum number of iterations configured for this agent
...
**[4] `gen_ai.agent.token_budget.consumed`:** Total tokens (input + output) consumed across all inference calls
...
**[5] `gen_ai.agent.token_budget.limit`:** The cumulative token budget (input + output, across all inference
...
**[6] `gen_ai.conversation.compacted`:** This attribute is a positive indicator of context compaction. Instrumentations
...
**[38] `gen_ai.usage.output_tokens`:** When the provider reports both billed token counts and model-consumed
token counts (for example, Cohere exposes both `usage.billed_units` and
`usage.tokens`), instrumentations SHOULD report the billed count so the
value matches the units the customer is charged for.

**[39] `gen_ai.usage.reasoning.output_tokens`:** The value SHOULD be included in `gen_ai.usage.output_tokens`.

**[40] `gen_ai.workflow.name`:** This attribute can be populated in different frameworks; for example, as the name of the first chain in LangChain or the name of the crew in CrewAI.
