...
**[12] `gen_ai.token.cache`:** This attribute MUST only be set when `gen_ai.token.type` is `input`.
...
**[13] `gen_ai.token.reasoning`:** This attribute MUST only be set when `gen_ai.token.type` is `output`.
...
**[22] `gen_ai.workflow.name`:** This attribute can be populated in different frameworks; for example, as the name of the first chain in LangChain or the name of the crew in CrewAI.
