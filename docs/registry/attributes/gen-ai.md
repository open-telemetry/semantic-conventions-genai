...
| <a id="gen-ai-attribution-link-type" href="#gen-ai-attribution-link-type">`gen_ai.attribution.link_type`</a> | ![Development](https://img.shields.io/badge/-development-blue) | string | Semantic type of an OTel span link that records attribution or provenance relationships between GenAI pipeline spans. [2] | `CAUSED_BY_GENERATION`; `RETRY_OF`; `INFORMED_BY` |
...
**[2] `gen_ai.attribution.link_type`:** This attribute SHOULD be recorded on span links, not on the span itself.
It classifies the relationship between the span that carries the link and
the linked span.

Instrumentations that support structured tool calls (i.e., the LLM returns
tool invocations with stable `tool_call_id` values) SHOULD use
`CAUSED_BY_GENERATION` links on `execute_tool` spans to enable backends to
reconstruct which generation triggered each tool invocation without
relying on parent-child hierarchy alone.

See [Attribution via span links](/docs/gen-ai/gen-ai-agent-spans.md#attribution-via-span-links)
...
`gen_ai.attribution.link_type` has the following list of well-known values. If one of them applies, then the respective value MUST be used; otherwise, a custom value MAY be used.

| Value | Description | Stability |
| --- | --- | --- |
| `CAUSED_BY_GENERATION` | The linked span is the LLM generation (chat span) whose completion contained the tool-call invocation that triggered this span. Used on `execute_tool` spans to record which generation caused the tool call. Addresses the open question in [open-telemetry/semantic-conventions-genai#309](https://github.com/open-telemetry/semantic-conventions-genai/issues/309). | ![Development](https://img.shields.io/badge/-development-blue) |
| `INFORMED_BY` | The linked span is a tool-call result that informed a subsequent LLM generation. Records the provenance of retrieved context or tool output that influenced a chat span. | ![Development](https://img.shields.io/badge/-development-blue) |
| `RETRY_OF` | The linked span is the previous attempt of the same operation that was retried. Used to chain retry attempts so the full retry history is reconstructable from the trace without relying on span attributes alone. | ![Development](https://img.shields.io/badge/-development-blue) |
