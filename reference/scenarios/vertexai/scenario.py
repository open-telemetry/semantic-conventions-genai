    ...
    with warnings.catch_warnings():
        ...
        with _reference_tracer.start_as_current_span("chat gemini-2.0-flash", attributes=span_attributes) as span:
            ...
            if hasattr(first_response, "usage_metadata") and first_response.usage_metadata:
                first_usage = _usage_attributes(first_response.usage_metadata)
                for attr, value in first_usage.items():
                    span.set_attribute(attr, value)
            ...
            event_attrs.update(first_usage)
            reference_event_logger().emit(
                event_name="gen_ai.client.inference.operation.details",
                body="Inference operation details",
                attributes=event_attrs,
        ...
        with _reference_tracer.start_as_current_span("chat gemini-2.0-flash", attributes=span_attributes) as span:
            ...
            if hasattr(second_response, "usage_metadata") and second_response.usage_metadata:
                second_usage = _usage_attributes(second_response.usage_metadata)
                for attr, value in second_usage.items():
                    span.set_attribute(attr, value)
            ...
            event_attrs.update(second_usage)
            reference_event_logger().emit(
                event_name="gen_ai.client.inference.operation.details",
                body="Inference operation details",
                attributes=event_attrs,
