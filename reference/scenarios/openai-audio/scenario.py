...
that audio token usage (`gen_ai.usage.audio.input_tokens` /
`gen_ai.usage.audio.output_tokens`) and audio-modality messages are capturable
    ...
    with _reference_tracer.start_as_current_span("chat gpt-4o-audio-preview", attributes=span_attributes) as span:
        ...
        if usage:
            ...
            if input_details and input_details.audio_tokens:
                span.set_attribute("gen_ai.usage.audio.input_tokens", input_details.audio_tokens)
            output_details = usage.completion_tokens_details
            if output_details and output_details.audio_tokens:
                span.set_attribute("gen_ai.usage.audio.output_tokens", output_details.audio_tokens)
